...
到本仓库右侧 **Releases** 最新版（v0.1.1）下载，只有两个文件，平台不通用：

| 文件 | 支持平台 | 用途 |
| --- | --- | --- |
| `DeepSeek-Harness-0.1.1-Setup.exe` | **仅 Windows**（Win10/11 x64） | 电脑端**安装包**，双击安装，自动创建桌面快捷方式 |
| `DeepSeek-Harness-0.1.1-mobile.apk` | **仅 Android**（无需 Google 服务） | 手机端 App（约 32MB），传到手机安装 |
...
手机连电脑按 **局域网 → IPv6 直连 → Cloudflare 隧道** 的顺序自动优选，尽量少走中转：

- **局域网**：同一 WiFi 最快，走内网直连。
- **IPv6 直连**：电脑有公网 IPv6（如移动宽带 2408:/64）时，手机可直接连电脑，**无中转、延迟最低**；
  电脑端“不同网络”二维码会**优先生成 IPv6 直连地址**，无需等隧道就绪。
- **Cloudflare 隧道**：以上都不通时兜底，免费免配置、无需公网 IP；隧道地址每次重启会变，重新扫码即可。

手机端会在网络切换（WiFi → 流量、跨网段）时**自动重连**；连接失败自动降级到下一个可用地址。
...
- **手机端**
  ...
  - **运行状态在对话流里**：agent 工作时消息底部显示“📥 接收信息中… / 🤔 思考中…”，
    有产出文本时显示“接收信息中”，否则显示“思考中”，不再放顶部菜单栏
  - **发送去重**：发出消息后本地先显示“发送中”，服务端回显到达的瞬间自动移除本地气泡，
    只留一条真实消息（不再出现“两条相同消息”一闪而过）；带图发送时回显会自动补齐图片
  - 斜杠命令（`/compact` 压缩上下文、`/permission` 权限模式、`/preset` Agent 预设、`/stop` 停止等，
    与桌面端同一数据源）；执行长命令时显示友好动作名（如“正在压缩上下文…”）
    - **慢命令不重复执行**：`/m/command` 放宽超时且**禁止超时自动重发**，
      避免 `/compact` 这类耗时操作被当成网络故障而“执行两遍”（第二次会撞到引擎维护槽忙）
  - **问题组交互**：agent 提问的整组子问题（单选/多选/自定义输入）逐题作答、可跳过，
    单题组点击即提交；提交后服务端接受才移除本地卡片
  - 超长回复自动折叠，点“展开全部”查看
...
- **远程访问连不上？** 部分网络会屏蔽 `trycloudflare.com`，可先试手机流量；有公网 IPv6 时可走直连，不行再用“同一网络”。
- **电脑重启后远程地址变了？** Cloudflare 隧道地址每次重启会变，重新扫码一次即可（手机 App 会记住电脑，局域网连接不受影响）。
- **点 `/compact` 报“Compaction is unavailable … not idle”？** 说明电脑端已有一次压缩在进行（引擎并发保护），
  手机上那次会被拒绝一次；用最新版后新发的 `/compact` 只会真正触发一次，不会重复执行。
...
> `apk\capacitor.config.json` 已内置（生产配置直接可用）；`xcopy` 会把 `..\mobile-ui\` 的界面源码
> （`app.js`/`index.html`/`style.css`）刷进 `apk\www\`，再 `cap copy android` 打进 APK。
