...
- **安装体验**
  ...
  - 启动即先显示**真实界面骨架**（左侧会话列表 / 聊天区 / 输入框全部显示“加载中”），
    服务在后台启动；等待期间动态显示“已等待 N 秒”，不再看起来像卡死
  - **冷启动提速**：安装器（管理员时）自动把应用目录加入 Windows Defender 排除，
    避免实时扫描上万个小文件；桌面端也可在“帮助 → 优化启动速度”手动添加
...
cd dsh-desktop
npm install
.\build-win.cmd        # 产出发行包（自动使用国内镜像下载构建工具，不依赖梯子）
